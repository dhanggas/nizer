...
public class LaporanSirkulasiAsetView extends javax.swing.JInternalFrame {
    ...
    private class JumlahAset {
        ...
        public JumlahAset(Penduduk aset, Integer jumlah) {
            this.aset = aset;
            this.jumlah = jumlah;
        }
    ...
    private void refreshDataTable() {
        ...
        for (SirkulasiAset sirkulasi : daftarSirkulasiAset) {
            Penduduk brg = sirkulasi.getAset();
            Object[] row = {
                brg.getNik(),
                brg.getNama(),
//                brg.getKategoriAset().getNama_kategori(),
    ...
    private List<JumlahAset> getAsetMasukPengembalian(List<PengembalianDetail> daftarTambah) {
        ...
        for (Penduduk aset : daftarAset) {
            ...
            for (PengembalianDetail tambah : daftarTambah) {
                Penduduk a = tambah.getAset();
                if (aset.getNik().equalsIgnoreCase(a.getNik())) {
    ...
    private List<JumlahAset> getAsetkeluarPeminjaman(List<PeminjamanDetail> daftarHapus) {
        List<JumlahAset> list = new ArrayList<>();
        for (Penduduk barang : daftarAset) {
            Integer jumlah = 0;
            for (PeminjamanDetail hapus : daftarHapus) {
                Penduduk b = hapus.getAset();
                if (barang.getNik().equalsIgnoreCase(b.getNik())) {
                    jumlah += hapus.getQty();
                }
            }
    ...
    public Integer jumlah(Penduduk aset, List<JumlahAset> daftar) {
        Integer hasil = 0;
        for (JumlahAset data : daftar) {
            if (data.getAset().getNik().equalsIgnoreCase(aset.getNik())) {
