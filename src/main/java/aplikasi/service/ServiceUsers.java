...
public class ServiceUsers implements RepoUsers {
    ...
    public Users save(Users value) throws SQLException {
        String sql = "INSERT INTO tb_user (id_user, nama, username, password, jabatan, status) VALUES (?, ?, ?,?,?,?)";
    ...
    public Users update(Users value) throws SQLException {
        String sql = "UPDATE tb_user SET nama = ?, username = ?, password = ?,jabatan = ?, status = ? WHERE id_user = ?";
        Connection connect = ds.getConnection();
        PreparedStatement ps = connect.prepareStatement(sql);
    ...
    public List<Users> findMaxValue() throws SQLException {
        String sql = "SELECT * FROM tb_user order BY id_user DESC LIMIT 1";
        List<Users> list = new ArrayList<>();
    ...
    public List<Users> findByNama(String nama) throws SQLException {
        String sql = "SELECT \n"
                ...
                + "    p.jabatan as jabatan, \n"
                + "    p.status as status \n"
                + "FROM tb_user p where nama like CONCAT('%', ?, '%')";
        List<Users> list = new ArrayList<>();
    ...
    @Override
    public Users verifikasi(Users value) throws SQLException {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
